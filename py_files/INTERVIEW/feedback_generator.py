...
class FeedbackGenerator:
    ...
    def generate_final_report(self, interview_data):
        ...
        total_content = sum(
            qa["analysis"].get("content_relevance", 0) for qa in qa_pairs
        ) / len(qa_pairs)
        total_clarity = sum(qa["analysis"].get("clarity", 0) for qa in qa_pairs) / len(
            qa_pairs
        )
        total_completeness = sum(
            qa["analysis"].get("completeness", 0) for qa in qa_pairs
        ) / len(qa_pairs)
        total_star = sum(qa["analysis"].get("star_method", 0) for qa in qa_pairs) / len(
            qa_pairs
        )
        total_technical = sum(
            qa["analysis"].get("technical_depth", 0) for qa in qa_pairs
        ) / len(qa_pairs)
        total_pace = sum(
            qa["speech_metrics"].get("pace_score", 70)
            for qa in qa_pairs
            if "speech_metrics" in qa
        ) / len(qa_pairs)
        ...
        practice_areas = self._generate_practice_recommendations(
            improvements, performance_by_difficulty
        )
    ...
    def _identify_strengths(self, qa_pairs, avg_scores):
        ...
        if avg_scores["content_relevance"] >= 80:
            strengths.append(
                "Strong ability to address questions directly and relevantly"
            )
    ...
    def _identify_improvements(self, qa_pairs, avg_scores):
        ...
        if avg_scores["pace"] < 60:
            improvements.append(
                "Adjust speaking pace - aim for 130-170 words per minute"
            )
    ...
    def _generate_practice_recommendations(
        self, improvements, performance_by_difficulty
    ):
        ...
        if performance_by_difficulty.get("easy", 0) < 70:
            recommendations.append(
                "Practice fundamental concepts and common interview questions"
            )

        if performance_by_difficulty.get("medium", 0) < 65:
            recommendations.append(
                "Focus on scenario-based questions and problem-solving"
            )

        if performance_by_difficulty.get("hard", 0) < 60:
            recommendations.append(
                "Study advanced topics and complex technical scenarios"
            )

        # Based on improvements needed
        if any("STAR" in imp for imp in improvements):
            recommendations.append(
                "Practice behavioral questions using STAR method (Situation, Task, Action, Result)"
            )

        if any("technical" in imp.lower() for imp in improvements):
            recommendations.append(
                "Review technical concepts and work on hands-on projects"
            )
        ...
        return (
            recommendations[:4]
            if recommendations
            else ["Keep practicing interview skills"]
        )
