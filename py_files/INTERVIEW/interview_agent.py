...
class InterviewAgent:
    ...
    def analyze_answer(
        self, question, answer, difficulty, resume_context="", jd_context=""
    ):
        """
  ...
  "feedback": "Brief overall feedback"
    ...
    def calculate_speech_metrics(self, answer_text, duration_seconds):
        ...
        if 130 <= words_per_minute <= 170:
            ...
            pace_score = 60
            pace_feedback = (
                "Pace is a bit slow" if words_per_minute < 130 else "Pace is a bit fast"
            )
        else:
            pace_score = 40
            pace_feedback = (
                "Pace needs improvement - too slow"
                if words_per_minute < 90
                else "Pace needs improvement - too fast"
            )
    ...
    def generate_follow_up_question(
        self, original_question, answer, interview_context=""
    ):
        """
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an interviewer asking insightful follow-up questions.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.7,
