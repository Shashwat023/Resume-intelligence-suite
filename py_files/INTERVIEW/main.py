...
@app.post(
    "/api/interview/setup", response_model=InterviewSetupResponse, tags=["Interview"]
)
async def setup_interview(
    ...
    try:
        ...
        if not resume_text or len(resume_text) < 50:
            ...
            raise HTTPException(
                status_code=400, detail="Resume text is too short or empty"
            )

        if not jd_text or len(jd_text) < 50:
            cleanup_file(resume_path)
            raise HTTPException(
                status_code=400, detail="Job description text is too short or empty"
            )
        ...
        session_id = create_session(
            candidate_name, position, resume_text, jd_text, questions
        )
...
@app.get(
    "/api/interview/question/{session_id}",
...
@app.post(
    "/api/interview/answer", response_model=AnswerAnalysisResponse, tags=["Interview"]
)
async def submit_voice_answer(
    session_id: str = Form(..., description="Session UUID"),
    question_number: int = Form(..., description="Question number being answered"),
    audio_answer: UploadFile = File(
        ..., description="Audio file with spoken answer (WAV, MP3, M4A)"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Answer submission failed: {str(e)}"
        )
...
async def get_interview_report(session_id: str):
    ...
    try:
        ...
        if len(session["answers"]) == 0:
            raise HTTPException(
                status_code=400,
                detail="No answers submitted yet. Complete the interview first.",
            )
