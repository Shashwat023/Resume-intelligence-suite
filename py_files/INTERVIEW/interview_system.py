...
class AIMockInterviewSystem:
    ...
    def setup_interview(self, resume_path, jd_path, candidate_name="", position=""):
        ...
        try:
            self.resume_text, self.jd_text = self.parser.parse_resume_and_jd(
                resume_path, jd_path
            )
            ...
            self.questions = self.question_gen.generate_questions(
    ...
    def conduct_interview(self, max_answer_duration=60):
        ...
        for i, q_data in enumerate(self.questions, 1):
            ...
            audio_file = self.recorder.record_audio(
                duration=max_answer_duration, output_file=f"answer_{i}.wav"
            )
            end_time = time.time()
...
def main():
    ...
    print(
        f"   - {sum(1 for q in system.questions if q['difficulty'] == 'easy')} Easy questions"
    )
    print(
        f"   - {sum(1 for q in system.questions if q['difficulty'] == 'medium')} Medium questions"
    )
    print(
        f"   - {sum(1 for q in system.questions if q['difficulty'] == 'hard')} Hard questions"
    )
