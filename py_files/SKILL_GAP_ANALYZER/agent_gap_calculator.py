...
class GapCalculatorAgent:
    ...
    def __init__(self):
        ...
        self.llm = ChatGroq(
            api_key=self.groq_api_key, model="llama-3.3-70b-versatile", temperature=0.1
        )
    ...
    def deduplicate_skills(self, skills: List[Dict]) -> List[Dict]:
        ...
        for skill in skills:
            ...
            if (
                ...
                or skill["proficiency"] > seen[normalized]["proficiency"]
            ):
                if normalized in seen:
                    ...
                    unique_skills = [
                        ...
                        if self.normalize_skill_name(s["name"]) != normalized
                    ]
    ...
    def filter_redundant_skills(
        self, required_skills: List[Dict], candidate_skills: List[Dict]
    ) -> List[Dict]:
        """
    ...
    def calculate_gaps(
        self, candidate_skills: List[Dict], required_skills: List[Dict]
    ) -> Dict:
        ...
        required_skills = self.filter_redundant_skills(
            required_skills, candidate_skills
        )

        # Create skill name mappings for fuzzy matching
        candidate_map = {
            self.normalize_skill_name(s["name"]): s for s in candidate_skills
        }
        required_map = {
            self.normalize_skill_name(s["name"]): s for s in required_skills
        }
        ...
        for req_skill_name, req_skill in required_map.items():
            ...
            if matched_cand_skill:
                proficiency_gap = (
                    req_skill["proficiency"] - matched_cand_skill["proficiency"]
                )
    ...
    def enhance_gap_analysis(
        self, gaps: List[Dict], resume_text: str, job_text: str
    ) -> List[Dict]:
...
Enhance these gaps with importance and context.""",
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            print(
                f"  → Raw input: {len(candidate_skills)} candidate skills, {len(required_skills)} required skills"
            )
            ...
            print(
                f"  ✓ After deduplication: {len(gap_analysis['gaps'])} unique skill gaps"
            )
            ...
            # Print critical gaps
            critical_gaps = [
                g for g in gap_analysis["gaps"] if g["gap_severity"] == "critical"
            ]
            if critical_gaps:
