...
class SkillExtractorAgent:
    ...
    def __init__(self):
        ...
        if not self.groq_api_key:
            raise ValueError(
                "GROQ_API_KEY not found in environment variables. Please add it to .env file"
            )
    ...
    def extract_skills_with_llm(
        self, text: str, context: str, max_retries: int = 3
    ) -> List[SkillItem]:
...
""",
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    raise Exception(
                        f"Failed after {max_retries} attempts. Error: {str(e)}"
                    )
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if candidate_skills:
                print(
                    f"  → Sample candidate skills: {[s['name'] for s in candidate_skills[:5]]}"
                )
            if required_skills:
                print(
                    f"  → Sample required skills: {[s['name'] for s in required_skills[:5]]}"
                )
            ...
            if "Connection" in error_msg or "API" in error_msg:
                ...
                state["errors"] = [
                    "SkillExtractor: API connection failed. Please check your GROQ_API_KEY and internet connection."
                ]
            else:
