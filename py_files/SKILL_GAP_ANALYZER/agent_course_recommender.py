...
class CourseRecommenderAgent:
    ...
    def __init__(self):
        ...
        self.llm = ChatGroq(
            api_key=self.groq_api_key, model="llama-3.3-70b-versatile", temperature=0.3
        )
...
[
  {{
    ...
    def create_learning_roadmap(
        self, gaps: List[Dict], courses: List[CourseRecommendation]
    ) -> Dict:
        ...
        sorted_gaps = sorted(
            gaps, key=lambda x: severity_order.get(x["gap_severity"], 4)
        )
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            priority_gaps = [
                g for g in unique_gaps if g["gap_severity"] in ["critical", "high"]
            ][:10]

            for i, gap in enumerate(priority_gaps, 1):
                print(
                    f"  → [{i}/{len(priority_gaps)}] Finding courses for {gap['skill']}..."
                )
            ...
            print(
                f"  ✓ Total estimated learning time: {total_time:.1f} hours ({total_time / 40:.1f} weeks at 40h/week)"
            )
            ...
            if all_courses:
                print(
                    f"  → Sample courses: {[c['course_title'][:50] for c in all_courses[:3]]}"
                )
